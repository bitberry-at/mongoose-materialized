...
function materialized(schema, options) {
    ...
    schema.pre('save', function (next, done) {
        ...
        if (!self.isNew && isPidChange) {
            ...
            if (!self.parentId) {
                ...
                self.collection.findOne(byId(self.parentId), addId({path: 1}), function(err, newParent){
                    if (err || !newParent){
                        self.invalidate('parentId', 'Parent not found!');
                        return done(new Error('Parent not found!'));
                    }
    ...
    schema.static('ToArrayTree', function (docs, fields) {
        ...
        for (var i=0, len=docs.length; i < len; i++) {
            ...
            let origEl = docs[i].toObject({ virtuals: false })
            let title = origEl.title;
            let slug = origEl.slug;
            ...
            // var el = docs[i].toObject({virtuals: false});
            if (el.parentId) {
                map[i] = {
                    index: i,
                    from : el[options.field].toString(),
